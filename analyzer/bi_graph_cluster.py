...
output_file = 'output/bi_gram_cluster.csv'
...
CONS_CLUSTER = "(?<!্)[ক-নপ-রলশ-হড়-য়]্[ক-নপ-রলশ-হড়-য়]"
vowel = "[ািীুূৃে-ৌ]"
...
for i in range(length):
    word = word_list[i]
    #freq = freq_list[i]
    cluster_list = re.findall(CONS_CLUSTER+vowel, word)
    #print(cluster_list)
    add_to_dict(cluster_dict,cluster_list,1)
    #print(cluster_list)
    #print(cluster_dict)
...
for bigram in bigram_list:
    ...
    for word in word_list:
        ...
        if bool(re.search(bigram,word)) and word not in examples:
